...
class User extends CI_Controller {
	...
	public function is_logged_in()
	{
		  ...
		  //$user = $_SESSION['user'];		  
		  $f = (array)json_decode($_COOKIE['user']);
		$user = $f['uid'];
		  return isset($user);
	}
	...
	public function routeDashboard()
	{
		...
		//$flogin = $_SESSION['user']['first_login'];
		$f = (array)(json_decode($_COOKIE['user']));
		//var_dump($_COOKIE);exit;
		$flogin = $f['first_login'];
		if($flogin == 1){
	...
	public function uploadSpecificFile($files,$empid,$it){
		...
		if(empty($empid))
		{
			...
			if($it){//retry
				...
				//var_dump($spdf->split_pdf($files['tmp_name'][0], 'split/',$files['name'][0],$it));exit;
			}
