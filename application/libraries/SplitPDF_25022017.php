			...
			if($folders[0] == 0  && !empty($folders)){
				...
				for($i=0;$i<$d3c;$i++){
					if($i%9 == 0)
					{
						...
						if($n !="For EMPLOYEE'S RECORDS" && $n !='name, address, and ZIP' && $n !="For EMPLOYEE'S RECORDS (See" && $n !="Copy 2 To Be")
						{
							$narr[] = $n;
						}
						$tte = (string)(trim($n));
...
//var_dump($folders);exit;		
		...
		else if(count($re) > 0 && !$it){ //first uploads
			$skip = false;
		}		
		...
		if(($format == 1 || $format == 2)&& $skip )
		{
			...
			$aaa = explode(".",$fname);
			$fpath = $fname;
			$return = array("value"=>false,"retry"=>1,"fpath"=>$fpath);
