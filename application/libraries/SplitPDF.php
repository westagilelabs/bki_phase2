...
class SplitPDF {	
	...
	function split_pdf($filename, $end_directory = false,$fname,$it = false)
	{
		...
		//OFE EXP FIle
		//$data1 = explode("Bank Acct: OFFEXP627",$data);

		//AP EFT EXP FIle
		$data1 = explode("Mth",$data);
		$data22 = explode("Trans",$data);
		$d2c = count($data22);
		$dc = count($data1);
		//echo "<pre>";var_dump($data22);exit;
		if($dc > 1){
...
//var_dump($file_names);exit;
		...
		if($format == 1){
			for($i=0;$i<$dc; $i++){
				if($dc % 2 !=0){ //for odd indexes (We are getting names in odd indexed array's)
					$d1 = preg_split('/\s/',trim($data1[$i]));					
					//echo "<pre>";var_dump($data1[$i]);
					...
					if(!empty($eid))
					{
						...
						if($name != 'EFT Transmission' && $name != '20 Locality' && $name != '20 Locality ' && $name !='7261860XI GA' && $name != 'EFT TransmissionBatchelor')
							$file_names[] = $name;
					}
				}			
			}
			...
			for($i=1;$i<=$d2c; $i++){
				$d1 = preg_split('/\s/',trim($data1[$i]));					
				//echo "<pre>";var_dump($data1[$i]);
				$nid_arr = explode("Trans ",$data1[$i]);
				//echo "<pre>";var_dump($nid_arr);
				$nidstr = trim($nid_arr[1]);
				$nid = intval(substr($nidstr, 0, 4));
				$a[0] = $nid;
				$rre = $this->CI->usermodel->checkEmpIds($a);
				
				if(count($rre) > 0){				
					if($d2c % 2 !=0){ //for odd indexes (We are getting names in odd indexed array's)
						///lets assume first name and last name we got here
						
						$name = trim($d1[0]." ".$d1[1]." ".$d1[2]);
						//var_dump($name);
						if($name != 'EFT Transmission')
							$eid = $this->CI->usermodel->getEmpIdByName($name);						
...
//var_dump($folders);var_dump($format);var_dump($file_names);
//exit;	
		try{
			$pdf = new FPDI();
		...
		if(($format == 1 || $format == 2 || $format == 3)&& $skip )
		{
			$i=0;
			//var_dump($folders);
			...
			foreach ($folders as $fk=>$fv) {
				...
				try{
					...
					//echo "<pre>";echo "$fk";var_dump($e);exit;
				}
