...
//	DELETE FOLDER LIST ITEM
	$(document).on('click', '.delete-icon', function(){
		...
		var delete_row_item_id = $(this).parent('.superadmin-user-delete').parent('li').index();
		...
		if(class_name.indexOf('delete_folder') != -1){
			...
			$("#empid").parent('form').attr('action','deleteListItem/E');
			$('#record-index').val(delete_row_item_id);
			$('#confirm').on('click', function(){
				$('.popup').hide();
				$('html').removeClass('hide-body');
				$('.popup').removeClass('display-popup');
